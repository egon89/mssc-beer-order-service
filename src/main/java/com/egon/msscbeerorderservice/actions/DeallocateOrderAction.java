...
import com.egon.brewery.dtos.events.DeallocateBeerOrderRequest;
...
public class DeallocateOrderAction implements Action<OrderStatusEnum, OrderEventEnum> {
  ...
  public void execute(StateContext<OrderStatusEnum, OrderEventEnum> stateContext) {
    ...
    log.debug("Deallocate order request action for order {}", orderId);
    ...
    final var message = DeallocateBeerOrderRequest.builder()
        .beerOrderDto(mapper.toDto(beerOrder))
        .build();
    try {
      jmsTemplate.convertAndSend(JmsConfig.DEALLOCATE_ORDER_QUEUE, message);
        log.debug("Order request {} sent to {} queue ", orderId, JmsConfig.DEALLOCATE_ORDER_QUEUE);
    } catch (Exception ex) {
        final var errorMessage = "Error to send order %s to %s queue".formatted(orderId, JmsConfig.DEALLOCATE_ORDER_QUEUE);
