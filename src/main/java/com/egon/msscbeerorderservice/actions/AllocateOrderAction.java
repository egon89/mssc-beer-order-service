...
public class AllocateOrderAction implements Action<OrderStatusEnum, OrderEventEnum> {
  ...
  public void execute(StateContext<OrderStatusEnum, OrderEventEnum> stateContext) {
    ...
    log.debug("Allocate order request action for order {}", orderId);
    ...
    final var message = AllocateBeerOrderRequest.builder()
        .beerOrderDto(mapper.toDto(beerOrder))
        .build();
    try {
      jmsTemplate.convertAndSend(JmsConfig.ALLOCATE_ORDER_QUEUE, message);
      log.debug("Order request {} sent to {} queue ", orderId, JmsConfig.ALLOCATE_ORDER_QUEUE);
    } catch (Exception ex) {
      final var errorMessage = "Error to send order %s to %s queue".formatted(orderId, JmsConfig.ALLOCATE_ORDER_QUEUE);
